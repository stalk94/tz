...
class Heap {
    private data: number[] = [];
    constructor(private cmp: (a: number, b: number) => boolean) { } // true если a «лучше» b

    push(x: number) { this.data.push(x); this.up(this.data.length - 1); }
...
class CountMinSketch {
    ...
    estimate(x: number) {
        return Math.min(...this.hashSeeds.map((seed, i) => {
            const idx = this.hash(x, seed);
...
export class Stats {
    ...
    getStats(): StatsResult {
        ...
        const mode = median; // временно псевдомода
